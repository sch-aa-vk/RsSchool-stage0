...
const player = document.querySelector('video__items');
const poster = document.querySelector('.poster__img');
const video = document.querySelector('.videoPlayer');
const playBtn = document.querySelector('.video__playHover');
const controls = document.querySelector('.player__controls');
const progress = document.querySelector('.progress__bar');
const volume = document.querySelector('.volume__slider');
const playIcon = document.querySelector('.play-icon__play');
const volumeIcon = document.querySelector('.play-icon__volume');

function openVideo() {
    poster.classList.add('click');
    video.classList.add('click');
    controls.classList.add('click');
}
...
function togglePlay() {
    if(video.paused) {
        ...
        playIcon.classList.add('click');
    } else {
        video.pause();
        playBtn.classList.toggle('click');
        playIcon.classList.remove('click');
    }
}
...
function volumeProgress(e) {
    const newVolume = (e.offsetX / volume.offsetWidth) * 100;
    volume.style.background = `linear-gradient(to right, #BDAE82 0%, #BDAE82 ${newVolume}%, #fff ${newVolume}%, white 100%)`;
    if(newVolume <= 2) {
        video.volume = 0;
        volumeIcon.classList.add('click');
    } else {
        video.volume = e.offsetX / volume.offsetWidth;
        volumeIcon.classList.remove('click');
    }
}
volume.addEventListener('click', volumeProgress);

volume.addEventListener('mousemove', (e) => mousedown && volumeProgress(e));
volume.addEventListener('mousedown', () => mousedown = true);
volume.addEventListener('mouseup', () => mousedown = false);

volumeIcon.addEventListener('click', () => {
    volumeIcon.classList.toggle('click');
    if(volumeIcon.classList.contains('click')) {
        video.volume = 0;
    } else {
